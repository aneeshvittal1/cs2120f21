-- Aneesh Vittal, AKV6ZR@virginia.edu
/- 
   *******************************
...
The introduction rule for true is that we can assume a proposition to be true
given a proof of the propsition.
...
This introduction rule says: If we have assume that P and Q are 
propositions, and we have proofs of both P and Q with p and q respectively,
we can obtain a proof of P ∧ Q 
...
The introduction rule for ∀ is that we assume that we are given an arbitrary object
of type T. Since t is of type T, our intruduction rule yields Q.
...
If we assume that we have an object t of type T and pass in a proof of t,
...
We can use the introduction rule for ∀ to apply pf to t and derive a proof of Q
-/
...
-/
example :  (KnowsLogic Lynn: BetterComputerScientist):= 
...
/-Proof By negation works by assuming a proof of P to be true, and
using contradiction to prove ¬P by showing that P → false
...
To prove P, assume ¬P and show that the assumptions causes a contradiction.
...
is assumed to be valid, and that accepting the axiom
...
English: The proposition states that everyone likes a nice, talented, person.
If John Lennon is a person, and hes talented and nice, then all people like him
-/
...
begin
  unfold ELJL,
  assume P N Talent Likes likeifTN,
  assume person nicenTalent,
